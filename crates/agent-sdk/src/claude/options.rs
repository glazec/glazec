...
use crate::claude::hooks::HookRegistry;
use crate::claude::mcp::SdkMcpServer;
...
pub struct ClaudeOptions {
    ...
    /// Hook callbacks invoked by the CLI at well-defined points in a turn.
    pub hooks: HookRegistry,
    /// In-process MCP servers whose tools are implemented in Rust and served
...
impl ClaudeOptions {
    ...
    /// `--mcp-config` JSON announcing the in-process servers, or `None` when
    /// there are none.
    fn mcp_config(&self) -> Option<Value> {
        ...
        let servers: serde_json::Map<String, Value> = self
            .mcp_servers
            .iter()
            .map(|server| (server.name.clone(), server.config_entry()))
            .collect();

        Some(json!({ "mcpServers": Value::Object(servers) }))
    }
...
mod tests {
    ...
    use crate::claude::mcp::{SdkMcpServer, SdkMcpTool, ToolResult};
    ...
    fn build_args_matches_expected_cli_shape() {
        ...
        options
            .extra_args
            .insert("replay-user-messages".to_string(), None);
        options
            .extra_args
            .insert("output-style".to_string(), Some("plain".to_string()));
    ...
    #[test]
    fn build_args_emits_sdk_mcp_config() {
        let options = ClaudeOptions {
            mcp_servers: vec![SdkMcpServer::new("calc").tool(SdkMcpTool::new(
                "add",
                "Add",
                json!({ "type": "object" }),
        ...
        let config: Value = serde_json::from_str(&args[index + 1]).expect("valid json");
