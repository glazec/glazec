        ...
        loop {
                ...
                .map_err(StdioError::StdoutRead)?
            {
                Some(line) => {
                    ...
                    let parsed =
                        serde_json::from_str(trimmed).map_err(|source| StdioError::DecodeJson {
...
mod tests {
    ...
    async fn test_stdio_roundtrip() {
        let mut client = Stdio::spawn(
            PathBuf::from("cat"),
            Vec::new(),
            StdioClientOptions::default(),
        )
        .expect("spawn cat");
