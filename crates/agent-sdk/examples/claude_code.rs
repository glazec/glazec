...
//!
//! `--demo-tools` registers an in-process MCP server (`mcp__demo__add`) and a
//! `PreToolUse` hook that logs every tool call to stderr — exercising the two
//! features this SDK now supports.
...
use agent_sdk::claude::{
    ...
    ControlResponseMessage, HookEvent, HookMatcher, HookOutput, PermissionMode, ResultMessage,
    SdkMcpServer, SdkMcpTool, StreamEventMessage, SystemMessage, ToolResult, UserMessage, hook,
};
use clap::Parser;
...
struct Cli {
    ...
    /// Register an in-process MCP server and a logging PreToolUse hook.
    #[arg(long)]
    demo_tools: bool,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cli.demo_tools {
        install_demo_tools(&mut options);
    }
...
/// Register an in-process MCP server plus a hook that observes tool calls.
fn install_demo_tools(options: &mut ClaudeOptions) {
    let server = SdkMcpServer::new("demo").tool(SdkMcpTool::new(
        "add",
    ...
    // Let the model actually call the in-process tool.
    options.allowed_tools.push("mcp__demo__add".to_string());

    options.hooks = options.hooks.clone().on(
        HookEvent::PreToolUse,
        HookMatcher::any(vec![hook(|input| async move {
            eprintln!("[hook] PreToolUse: tool={:?}", input.tool_name);
            HookOutput::new()
        })]),
    );
}
...
async fn stream_response(client: &mut Claude) -> Result<(), Box<dyn std::error::Error>> {
    loop {
        ...
        // Route hook_callback / mcp_message requests to the registered handlers;
        // unsupported subtypes are rejected by the SDK on our behalf.
        if let ClaudeMessage::ControlRequest(control_request) = &message {
            client.handle_control_request(control_request).await?;
            continue;
        }
